...
| [jev-baton](https://github.com/shitianfang/jev-baton) | Harness-agnostic handoff layer: MCP tools batch typed `Noul`/`Choice`/`Score` questions about agent state to Jev, application code owns thresholds, and a typed escalation contract returns low-confidence or untypeable steps to the LLM. | Repository |
